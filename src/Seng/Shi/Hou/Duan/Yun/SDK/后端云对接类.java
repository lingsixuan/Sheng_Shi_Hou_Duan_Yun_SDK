...
/**
 * API接口的具体实现。<br>
 * 此类需要有访问网络的权限，您需要在AndroidManifest.xml中声明网络权限。<br>
...
public class 后端云对接类 implements API, POST {
     ...
     * @param 项目ID    此处为你要对接的项目ID，可以在圣使后端云客户端中查看。<br>
     *                API靠项目ID来区分不同项目，<br>
     *                这个值不会改变。<br>
     * @param 摘要校验    这个值用来确定要不要开启数据包摘要检查，开启之后服务器会对下行数据附加摘要，会检查上行数据的摘要。<br>
     *                客户端会对上行数据附加摘要，会检查下行数据的摘要，<br>
     *                以此来保证数据没有被篡改。<br>
     *                如果启用此设置，服务器会强制要求所有上行数据必须附加摘要，<br>
     *                如果关闭此设置，则以上效果都不会生效。<br>
     *                这个值要和项目的设置一致才能正常工作。<br>
     * @param 摘要盐值    摘要校验的盐值，后端云通过这个盐值来保证通信安全。<br>
     *                在摘要校验启用时，数据包的摘要和校验都通过这个盐值进行，<br>
     *                盐值可以在APP内更换，但是更换盐值之后，没有同步更新的APP就无法和服务器建立连接了。<br>
     *                摘要要和项目的设置值一致才能正常工作。<br>
     */
     ...
     *                盐值可以在APP内更换，但是更换盐值之后，没有同步更新的APP就无法和服务器建立连接了。<br>
     *                摘要要和项目的设置值一致才能正常工作。<br>
     * @param 允许时间误差  SDK将检查收到的数据包中携带的时间戳，如果时间戳相对当前时间差距超过此阈值（单位秒），则不接受此数据包。<br>
     *                如果你的用户距离服务器（上海）比较远，或者网络波动较大，那么这个阈值需要设置的长一些。默认值30秒。<br>
     */
    ...
    /**
     * @param 当前版本号 服务器依靠版本号来判断当前请求的客户端版本和服务器上的众多版本之间的新旧关系。<br>
     *              当您在管理端发布新版本时，版本号一栏请务必填写和APP一样的版本号，且新的版本号必须比历史所有版本都大。<br>
     *              此参数建议和您项目的versionCode的值相同。<br>
     * @param 回调    回调方法，此值为null时，检查更新的请求不会实际提交到服务器。<br>
     */
    @Override
     ...
     * @param 账号 登录账户的账号。<br>
     * @param 密码 登录账户的密码。<br>
     * @param 回调 回调方法，请注意！回调方法为null时，登录请求并不会真的提交到服务器！<br>
     */
    @Override
